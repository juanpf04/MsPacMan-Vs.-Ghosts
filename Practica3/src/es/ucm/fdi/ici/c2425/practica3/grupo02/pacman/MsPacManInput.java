...
public class MsPacManInput extends RulesInput {
    ...
    public Collection<String> getFacts() {
        ...
        facts.add(String.format("(BLINKY (distance %d) (edible %s) (ghostEdibleTime %d) (ghostLairTime %d))",
            (int) this.distanceToBlinky,
            this.isBlinkyEdible ? "TRUE" : "FALSE",
            game.getGhostEdibleTime(GHOST.BLINKY),
            game.getGhostLairTime(GHOST.BLINKY)

        ));
        facts.add(String.format("(INKY (distance %d) (edible %s) (ghostEdibleTime %d) (ghostLairTime %d))",
            (int) this.distanceToInky,
            this.isInkyEdible ? "TRUE" : "FALSE",
        ...
        facts.add(String.format("(PINKY (distance %d) (edible %s) (ghostEdibleTime %d) (ghostLairTime %d))",
            (int) this.distanceToPinky,
            this.isPinkyEdible ? "TRUE" : "FALSE",
        ...
        facts.add(String.format("(SUE (distance %d) (edible %s) (ghostEdibleTime %d) (ghostLairTime %d))",
            (int) this.distanceToSue,
            this.isSueEdible ? "TRUE" : "FALSE",
            game.getGhostEdibleTime(GHOST.SUE),
            game.getGhostLairTime(GHOST.SUE)
        ));

        facts.add(String.format("(PACMAN (position %d) (last-move %s) (neightbour-nodes %d))", this.pacmanNode, this.lastMove, this.numberNeightbourNodes));

        // Añadir hecho con la distancia al punto de poder más cercano
        facts.add(String.format("(PILL (distance %d) (index %d))",
            this.distanceToNearestPill, this.indexPill
        ));

        facts.add(String.format("(POWERPILL (distance %d) (index %d))",
            this.distanceToNearestPowerPill, this.indexPowerPill
        ));
    ...
    public void parseInput() {
        ...
        this.distanceToBlinky = game.getDistance(game.getPacmanCurrentNodeIndex(), game.getGhostCurrentNodeIndex(GHOST.BLINKY), DM.MANHATTAN);
        this.distanceToInky = game.getDistance(game.getPacmanCurrentNodeIndex(), game.getGhostCurrentNodeIndex(GHOST.INKY), DM.MANHATTAN);
        this.distanceToPinky = game.getDistance(game.getPacmanCurrentNodeIndex(), game.getGhostCurrentNodeIndex(GHOST.PINKY), DM.MANHATTAN);
        this.distanceToSue = game.getDistance(game.getPacmanCurrentNodeIndex(), game.getGhostCurrentNodeIndex(GHOST.SUE), DM.MANHATTAN);
