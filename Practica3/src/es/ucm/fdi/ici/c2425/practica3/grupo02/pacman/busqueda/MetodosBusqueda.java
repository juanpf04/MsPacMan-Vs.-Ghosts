...
public class MetodosBusqueda {
	 ...
	 */
	public MetodosBusqueda(Game game) {
	 ...
	 */
	public MOVE getBestMove(int currentNode, MOVE lastMove) {
		this.update(game);

		currentNode = this.game.getPacmanCurrentNodeIndex();
		lastMove = this.game.getPacmanLastMoveMade();

		
		return doesMsPacManRequireAction(currentNode, lastMove) ? this.getBestPath(currentNode, lastMove, DEPTH).startMove : MOVE.NEUTRAL;
	...
	private PathInfo getBestPath(int currentNode, MOVE lastMove, int depth) {
		...
		for (MOVE move : possibleMoves) {
			...
			//int distanceToGhost = this.game.getShortestPathDistance(currentNode, this.game.getGhostCurrentNodeIndex(ghostToRunAwayFrom));
		...
		for (Ghost ghost : this.ghosts) {
			...
			if (ghost.lairTime == 0 && this.isCloseEnough(node, ghost.currentNodeIndex)) {
				...
				if (ghost.edibleTime == 0)
					...
					return (Constants.GHOST_EAT_SCORE * ++this.numGhostEaten * 200) + depth;
				}
			}
		}
