package es.ucm.fdi.ici.c2425.practica5.grupo02.CBRengine;
...
public class CustomPlainTextConnector implements Connector {
	...
	public void initFromXMLfile(URL file) throws InitializingException {
		try {
			...
			this.descriptionClass = Class
					.forName(doc.getElementsByTagName("DescriptionClassName").item(0).getTextContent())
					.asSubclass(CaseComponent.class);

			try {
				this.solutionClass = Class
						.forName(doc.getElementsByTagName("SolutionClassName").item(0).getTextContent())
						.asSubclass(CaseComponent.class);
			} catch (Exception e) {
			}

			try {
				this.justOfSolutionClass = Class
			...
			this.descriptionMaps = findMaps(doc.getElementsByTagName("DescriptionMappings").item(0),
			...
			if (this.justOfSolutionClass != null)
				this.justOfSolutionMaps = findMaps(doc.getElementsByTagName("JustificationOfSolutionMappings").item(0),
						this.justOfSolutionClass);
	...
	public void storeCases(Collection<CBRCase> cases) {
		try {
			...
			// if (br == null) throw new Exception("Error opening file for writing: "+
			// this.PROP_FILEPATH);
	...
	/**
	 * Deletes cases from the case base. It only uses the case name (primary key) to
	 * remove the row. Note that this method is very inefficient because it reads
	 * all the database, removes the rows in memory, and writes it again into the
	 * text file.
	 * 
	...
	public void deleteCases(Collection<CBRCase> cases) {
		try {
			...
			// InputStreamReader(FileIO.findFile(this.PROP_FILEPATH).openStream()));
			br = new BufferedReader(new InputStreamReader(FileIO.openFile(this.PROP_FILEPATH)));
			// if (br == null) throw new Exception("Error opening file for reading: " +
			// this.PROP_FILEPATH);
			...
			// FileWriter(FileIO.findFile(this.PROP_FILEPATH).getFile(), false));
			bw = new BufferedWriter(new FileWriter(FileIO.findFile(this.PROP_FILEPATH).getFile(), false));
			// if (bw == null) throw new Exception("Error opening file for writing: "+
			// this.PROP_FILEPATH);
			...
			LogManager.getLogger(this.getClass()).error("Error deleting cases " + e.getMessage());
		}
	}
	...
	public List<CBRCase> retrieveAllCases() {
		...
		try {
			BufferedReader br = null;
			br = new BufferedReader(new InputStreamReader(FileIO.openFile(this.PROP_FILEPATH)));
			// if (br == null) throw new Exception("Error opening file: " +
			// this.PROP_FILEPATH);
			...
			LogManager.getLogger(this.getClass()).error("Error retrieving cases " + e.getMessage());
		}
		return cases;
	}
	...
	private void fillComponent(CaseComponent component, StringTokenizer st, List<Attribute> maps, boolean includeId) {
		try {
			...
			LogManager.getLogger(this.getClass()).error("Error creating case: " + e.getMessage());
		}
	}
