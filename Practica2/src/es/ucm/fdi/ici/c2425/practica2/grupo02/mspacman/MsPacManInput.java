...
public class MsPacManInput extends Input {
	...
	public boolean pacmanEatPowerPill(){
        ...
        for(int PowerPill: this.getGame().getActivePowerPillsIndices()){
            sol = pacmanNode == PowerPill || Math.abs(pacmanNode - PowerPill) <= 2;
        }
    ...
    public boolean edibleGhostNearestThanGhost(int range) {
        return edibleGhostCloseDistance(range) < getEveryTypeClosestGhostDistance(range);
    }
		...
		if(range <= 40){
			...
			for(GHOST ghost: GHOST.values()){
				if(this.getGame().isGhostEdible(ghost)){
					int distance = this.getGame().getShortestPathDistance(currentNode, this.getGame().getGhostCurrentNodeIndex(ghost));
					
					if(distance < range){
						safestMove = this.getGame().getNextMoveAwayFromTarget(currentNode, this.getGame().getGhostCurrentNodeIndex(ghost), info.getPacmanLastMoveMade(), DM.PATH);
					}
				}
			}
		}
	...
	//Inicializa un conjunto con los nodos de los fantasmas comestibles
